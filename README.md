...
ガンマ関数は、発散する特異点を含まない正の定義域においても非常に大きな値となるので、[倍精度浮動小数点数](https://ja.wikipedia.org/wiki/倍精度浮動小数点数)（Double型）で計算したとしても、**約 171.7 以上の定義域** における[算術オーバーフロー](https://ja.wikipedia.org/wiki/算術オーバーフロー)が避けられない。
